...
- [ ] 4.1 实现基础对话 UI（气泡列表、输入框、发送按钮）
- [ ] 4.2 集成 Markdown 渲染库（flutter_markdown），支持代码高亮、表格、KaTeX 公式
- [ ] 4.3 实现对话历史分页加载与搜索功能
- [ ] 4.4 实现消息 Re-generate（分支重试）和分支切换 UI
- [ ] 4.5 实现上下文窗口自动压缩（摘要 + 分隔符提示）
- [ ] 4.6 实现图片输入（相机拍摄 / 相册选取 / 截图粘贴 / 桌面拖拽）
- [ ] 4.7 集成本地 OCR（Apple Vision / Google ML Kit）
- [ ] 4.8 实现发送图片至云端 LLM 前的隐私确认弹窗
...
- [ ] 5.1 定义 UIComponentRegistry 及 `UIComponentDefinition` 接口
- [ ] 5.2 实现 LLM 响应流 JSON Schema 解析器（提取 ui_type 块）
- [ ] 5.3 实现 `product_card` 组件（图片、价格、评分、操作按钮）
- [ ] 5.4 实现 `calendar_event` 组件（含"添加到日历"交互按钮）
- [ ] 5.5 实现 `contact_card` 组件（含"拨打"/"发邮件"交互按钮）
- [ ] 5.6 实现 `map_preview` 组件（静态地图图片 + 导航按钮）
- [ ] 5.7 实现 `flight_card` / `train_card` 组件
- [ ] 5.8 实现 `weather_card` 组件
- [ ] 5.9 实现 `code_block` 组件（高亮、复制、在沙箱中运行）
- [ ] 5.10 实现 `task_list` 组件（可勾选状态）
- [ ] 5.11 实现 `price_chart` 组件（时间轴折线图）
- [ ] 5.12 实现未知 ui_type 降级展示（格式化 JSON 代码块）
- [ ] 5.13 实现响应式布局适配（手机/平板/桌面）和深色模式自适应

## 6. 原生能力 Tool Bridge

- [ ] 6.1 定义 Tool 抽象接口及四级权限分级枚举（L0-L3）
- [ ] 6.2 实现权限状态持久化存储（L1 授权记录）
- [ ] 6.3 实现工具调用确认弹窗组件（L1 首次 / L2 每次 / L3 完整预览）
...
- [ ] 6.8 实现 URL Scheme 唤起地图（高德 / 百度 / Google Maps）、微信、支付宝、钉钉、飞书
- [ ] 6.9 实现系统工具：读取/写入剪贴板、系统分享菜单
- [ ] 6.10 实现对话界面工具调用状态实时显示（"正在调用：XXX…"提示）
- [ ] 6.11 实现设置页工具调用历史日志与权限撤销功能

## 7. Agent Skill 沙箱

- [ ] 7.1 实现 `SKILL.md` YAML frontmatter 解析器（name / description 校验）
...
- [ ] 8.10 实现任务管理 UI（任务列表 / 下次触发时间 / 执行历史日志）
...
- [ ] 9.5 实现 mDNS / Bonjour 局域网 Agent 自动发现
- [ ] 9.6 实现手动添加 Agent（二维码扫描 + URL 直接输入）
- [ ] 9.7 对接公开 A2A 注册目录，实现 Agent 浏览与搜索 UI
- [ ] 9.8 实现外部 Agent 调用前用户确认预览界面
- [ ] 9.9 实现 TLS 1.3 版本验证，低版本拒绝连接
- [ ] 9.10 实现外部 Agent 返回数据沙箱隔离处理（禁止宿主直接 eval）

## 10. 隐私与安全加固

- [ ] 10.1 集成本地轻量 NER 模型，实现手机号 / 身份证号 / 银行卡号 PII 检测
- [ ] 10.2 实现"发送前确认"模式 UI（Prompt 预览 + PII 高亮标注）
- [ ] 10.3 实现联系人姓名自动代号替换（会话级一致性）
...
- [ ] 10.7 确认 API Key 字段在同步序列化时严格排除
- [ ] 10.8 实现本地快照保留（同步前保存 7 天快照，防止数据丢失）

## 11. 设置中心与配置 UI

- [ ] 11.1 实现 LLM 提供商管理页（列表 / 添加 / 编辑 / 删除 / 可用性测试）
- [ ] 11.2 实现模型路由规则配置页
- [ ] 11.3 实现权限管理页（工具授权记录 / 撤销入口）
- [ ] 11.4 实现隐私设置页（PII 检测开关 / 数据保留天数 / 一键清除）
- [ ] 11.5 实现跨设备同步配置页（GitHub 授权 / passphrase 设置 / 同步状态）
- [ ] 11.6 实现可访问性支持（VoiceOver / TalkBack / 高对比度 / 字体大小跟随系统）
